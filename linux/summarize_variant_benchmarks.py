#!/usr/bin/env python3
"""Summarize aenet, AccelNet-direct, and AccelNet-moment LAMMPS logs."""
...
NAME_PATTERN = re.compile(
    ...
    r"(?P<mode>fixed|dynamic)-(?P<implementation>aenet|accelnet-direct|accelnet-moment)-"
    r"r(?P<ranks>\d+)-t(?P<trial>\d+)\.log"
)
...
def parse_log(path: Path):
    ...
    record = {**match.groupdict(), "path": str(path), "loop": float(loop), "steps": int(steps)}
    ...
    other_rows = re.findall(r"^Other\s*\|\s*\|\s*([0-9.eE+-]+)\s*\|", text, re.MULTILINE)
...
def main():
    ...
    if not records:
        raise SystemExit("no variant benchmark logs found")
    ...
    print("system\tsize\tatoms\tmode\timplementation\tranks\tsteps\ttrials\tloop_median_s\tloop_q1_s\tloop_q3_s\tsteps_per_s\tatom_steps_per_s\tpair_median_s\tneigh_median_s\tcomm_median_s\tmodify_median_s\tkspace_median_s\tother_median_s\tmax_dangerous_builds\tspeedup_vs_aenet\tspeedup_moment_vs_direct")
    medians = {key: statistics.median(record["loop"] for record in group) for key, group in groups.items()}
    for key in sorted(groups, key=lambda item: (item[0], item[2], item[3], item[5], item[4])):
        group = groups[key]
        loops = [record["loop"] for record in group]
        median_loop = medians[key]
        ...
        base = (key[0], key[1], key[2], key[3])
        aenet_key = (*base, "aenet", key[5], key[6])
        direct_key = (*base, "accelnet-direct", key[5], key[6])
        moment_key = (*base, "accelnet-moment", key[5], key[6])
        speedup_aenet = medians[aenet_key] / median_loop if aenet_key in medians else float("nan")
        moment_vs_direct = (
            medians[direct_key] / medians[moment_key]
            if direct_key in medians and moment_key in medians else float("nan")
        )
        print(
            ...
            + f"\t{speedup_aenet:.9g}\t{moment_vs_direct:.9g}"
        )
